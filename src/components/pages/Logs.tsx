    ...
    try {
      ...
      if (filePath) {
        const logContent = filteredLogs
          .map(log => `[${log.timestamp}] [隧道 ${log.tunnel_id}] ${log.message}`)
          .join('\n')
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Select
            ...
            onChange={(value) => setSelectedTunnelId(value === 0 ? null : Number(value))}
            placeholder="选择隧道"
            className="w-48"
          />
          ...
          <div className="p-4 font-mono text-[13px] space-y-1">
            {filteredLogs.length === 0 ? (
              <div className="text-muted-foreground">
                {selectedTunnelId ? "等待日志输出..." : "请选择一个隧道或启动隧道以查看日志"}
              </div>
            ) : (
              filteredLogs.map((log, index) => (
                <div key={index} className="text-foreground/90 hover:bg-foreground/5 px-2 py-0.5 rounded">
                  <span className="text-muted-foreground">[{log.timestamp}]</span>
                  {!selectedTunnelId && (
                    <span className="text-blue-500 ml-2">[隧道 {log.tunnel_id}]</span>
                  )}
